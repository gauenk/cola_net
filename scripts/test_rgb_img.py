...
def get_model(model_name,sigma,device):
    if model_name == "original":
        model = colanet.original.load_model("gray",sigma).to(device)
        return model
    elif model_name == "refactored":
        model = colanet.refactored.load_model("gray",sigma).to(device)
        return model
    elif model_name == "batched":
        model = colanet.batched.load_model("gray",sigma).to(device)
        return model
    else:
